...
export function Products() {
  return (
    <section id="boutique" className="relative overflow-hidden bg-dark-gradient py-24 text-ivory">
      <div
        aria-hidden
        className="pointer-events-none absolute inset-y-0 left-1/2 hidden w-px bg-gradient-to-b from-transparent via-gold/60 to-transparent opacity-40 lg:block"
      />
      <div className="container relative flex flex-col gap-16">
        <div className="grid items-center gap-10 rounded-panel border border-gold/20 bg-white/10 p-10 shadow-luxe backdrop-blur-lg lg:grid-cols-[1.2fr,1fr]">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {recharges.map((item, index) => (
              <Reveal
                key={item.name}
                delay={index * 0.05}
                className="flex flex-col gap-4 rounded-panel border border-gold/20 bg-white/10 p-6 text-ivory shadow-soft backdrop-blur"
              >
